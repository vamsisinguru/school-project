...
import { LayoutDashboard, Users, UserCog, ClipboardList, Bell, Calendar, Image, Plus, Edit, Trash2, MapPin } from 'lucide-react';
...
export default function AdminEventsPage() {
  ...
  const openCreate = () => { setEditing(null); setForm({ title: '', description: '', category: 'Cultural', startDate: '', venue: '', imageUrl: '' }); setModalOpen(true); };
  const openEdit = (e: Event) => { setEditing(e); setForm({ title: e.title, description: e.description, category: e.category, startDate: e.startDate.split('T')[0], venue: e.venue || '', imageUrl: e.imageUrl || '' }); setModalOpen(true); };

  const handleSave = async () => {
    if (!form.title || !form.description || !form.startDate) { toast('Title, description, and date required', 'error'); return; }
    setSaving(true);
    try {
      const url = editing ? `/api/staff/events/${editing.id}` : '/api/staff/events';
      const method = editing ? 'PUT' : 'POST';
      const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
      if (res.ok) { toast(editing ? 'Event updated!' : 'Event created!', 'success'); setModalOpen(false); fetchData(); }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { const res = await fetch(`/api/staff/events/${deleteId}`, { method: 'DELETE' }); if (res.ok) { toast('Event deleted!', 'success'); fetchData(); } }
    catch { toast('Error occurred', 'error'); }
  };
  ...
  return (
    <DashboardShell navItems={navItems} userName="" userRole="ADMIN" basePath="/portal/admin">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-navy-900">Events Management</h1><p className="text-sm text-navy-500 mt-1">Manage all school events.</p></div>
        <Button onClick={openCreate}><Plus className="h-4 w-4" /> Create Event</Button>
      </div>

      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2">{[...Array(4)].map((_, i) => <Skeleton key={i} className="h-48 w-full" />)}</div>
      ) : events.length === 0 ? (
        <Card><EmptyState icon={Calendar} title="No Events" /></Card>
      ) : (
      ...
      <ConfirmDialog open={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Event" message="Delete this event?" confirmText="Delete" danger />
    </DashboardShell>
  );
}
